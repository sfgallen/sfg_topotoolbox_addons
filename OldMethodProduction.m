function [Production] = OldMethodProduction(DEM,SHIELD,ICE,utm_zone,remove_glaciers,tag)
...
Pn= SHIELD.Z*Pn_SLHL.*(A.Z+B.Z.*exp(-pres/150)+C.Z.*pres+D.Z.*pres.^2+E.Z.*pres.^3); % Stone, 2000
Pms=SHIELD.Z*Pms_SLHL.*exp((1013.25-pres)/260); % 1013.25: sea level pressure, 260: muon attenuation lengths in the air(g/cm2) %Braucher et al., 2011
Pmf=SHIELD.Z*Pmf_SLHL.*exp((1013.25-pres)/510); % Braucher et al., 2011

% Calculate area production is computed over
was_used = ~isnan(SHIELD.Z);
