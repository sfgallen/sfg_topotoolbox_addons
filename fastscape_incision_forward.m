function [Z0,chi] = fastscape_incision_forward(S,S_DA,Ui,Uf,Ki,Kf,m,n,run_time,dt,nplot,varargin)
...
%
% optional input
% (12) chi - vector of chi for the STREAMobj.
% (13) Zi - user defined initial elevation of the river profile (e.g.
% extracted from the DEM). If no input is given an initial steady state
% river profile will be derived based on Ui and Ki.
%
% Outputs:
% (1) Z0 - the elevation of the river network at the end of the model run
% (2) chi
%
...
%   dt = 1e3;
%   nplot = 10;
%   [Z0,chi] = fastscape_incision_forward(S,S_DA,Ui,Uf,Ki,Kf,m,n,run_time,dt,nplot);
% 
...
% optional inputs
addOptional(p,'chi', 0, @(x) isvector(x));
